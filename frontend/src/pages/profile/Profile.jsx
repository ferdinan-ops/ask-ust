...
import {
  getMyPosts,
  getMySavedPosts,
} from "../../config/redux/features/userSlice";
...
const Profile = () => {
  ...
  useEffect(() => {
    dispatch(getMyPosts({ page, id }));
  }, [dispatch, page, id]);
  useEffect(() => {
    if (currentUserId === id) dispatch(getMySavedPosts(page));
  }, [dispatch, page, currentUserId, id]);
  useEffect(() => {
          ...
          <div className="profileWrapper">
            ...
            <div className="profileContents">
              <div className="profileTabs">
                <button
                  className={isSaveTabs ? "" : "active"}
                  onClick={() => setIsSaveTabs(false)}
                >
                  <QuestionMarkCircleIcon
                    className={isSaveTabs ? "icons" : "icons active"}
                  />
                </button>
                {currentUser === id && (
                  <button
                    className={isSaveTabs ? "active" : ""}
                    onClick={() => setIsSaveTabs(true)}
                  >
                    <BookmarkIcon
                      className={isSaveTabs ? "icons active" : "icons"}
                    />
                  </button>
                )}
              </div>
                ...
                {!isSaveTabs ? (
                  <>
                    {posts.length > 0 ? (
                      ...
                      <p className="nonePost">
                        <i>Belum ada pertanyaan</i>
                      </p>
                    )}
                    {posts.length > 2 && (
                      <InfiniteScroll
                  ...
                  currentUser === id && (
                    <>
                      {savedPosts.length > 0 ? (
                        ...
                        <p className="nonePost">
                          <i>Belum ada pertanyaan yang disimpan</i>
                        </p>
                      )}
                      {savedPosts.length > 2 && (
                        <InfiniteScroll
